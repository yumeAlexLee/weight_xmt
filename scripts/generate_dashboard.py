    ...
    <div class="card">
      ...
      <div class="form-grid">
        <div class="form-row-inputs">
          ...
          <div class="form-group">
            <label>体重 (kg)</label>
            <input type="number" id="input-weight" step="0.1" min="25" max="80" placeholder="45.0">
          </div>
        </div>
        <div class="form-group">
          <label>备注 (可选)</label>
    ...
    function addWeight() {{
      ...
      .then(function(data) {{
        if (data.content) {{
          ...
          // 即时更新图表
          var today = date;
          var shortDate = today.slice(5);
          chartData.labels.push(shortDate);
          chartData.rawDates.push(today);
          chartData.weights.push(parseFloat(weight));
          // 重建图表
          ...
          newRow.innerHTML = '<td>' + date + '</td><td>' + weight + '</td><td class="note">' + (note || '') + '</td>';
          tbody.insertBefore(newRow, tbody.firstChild);
          // 更新统计卡片
          var w = parseFloat(weight);
          var statsGrid = document.querySelector('.stats-grid');
          if (statsGrid) {{
            var lost = (chartData.weights[0] - w).toFixed(1);
            var remaining = (w - 45).toFixed(1);
            var cells = statsGrid.querySelectorAll('.value');
            if (cells.length >= 4) {{
              cells[0].textContent = w + 'kg';
              cells[1].textContent = (lost > 0 ? '+' : '') + lost + 'kg';
              cells[2].textContent = remaining + 'kg';
            }}
          }}
        }} else {{
